...
if __name__ == '__main__':
		...
		while True :                        				#boucle infinie
			neworder = random.sample(leds, 8)
			for i in range(len(neworder)) :
				print ("neworder : ",neworder,"led : ", neworder[i],"led -1 : ", neworder[i-1])
				GPIO.output(neworder[i], GPIO.HIGH)
				GPIO.output(neworder[i-1], GPIO.LOW)
				time.sleep(0.2)
